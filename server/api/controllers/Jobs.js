...
import schema from '../../../shared/models/schemas/job'
...
export default class JobsController extends RestController {
  constructor(options) {
    super(options.app, _.defaults({
      model_type: require('../../models/Job'),
      route: '/api/jobs',
      auth: [...options.auth, createAuthMiddleware({canAccess})],
      templates: {
        base: require('../templates/jobs/base'),
        detail: require('../templates/jobs/detail'),
      },
  ...
  places(req, res) {
    ...
    queue.defer(callback => Job.cursor({$unique: 'location', $values: 'location'}).toJSON((err, _locations) =>
